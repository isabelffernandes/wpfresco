	...
	class BSF_Analytics {
			...
			Astra_Notices::add_notice(
				array(
					...
					'message'                    => sprintf(
						...
						sprintf( $notice_string . '<span dir="%2s"><a href="%3s" target="_blank" rel="noreferrer noopener">%4s</a><span>', esc_html( $this->get_product_name() ), $language_dir, esc_url( $this->usage_doc_link ), __( ' Know More.', 'ultimate-addons-for-gutenberg' ) ),
						add_query_arg(
							array(
		...
		public function every_two_days_schedule( $schedules ) {
			$schedules['every_two_days'] = array(
				...
				'display'  => __( 'Every two days', 'ultimate-addons-for-gutenberg' ),
			);
		...
		public function render_settings_field_html() {
			...
			<label for="bsf-analytics-optin">
				...
				<?php
				esc_html_e( 'Allow Brainstorm Force products to track non-sensitive usage tracking data.', 'ultimate-addons-for-gutenberg' );
			...
			<?php
			echo wp_kses_post( sprintf( '<a href="%1s" target="_blank" rel="noreferrer noopener">%2s</a>', esc_url( $this->usage_doc_link ), __( 'Learn More.', 'ultimate-addons-for-gutenberg' ) ) );
			?>
			</fieldset>
			<?php
		}
